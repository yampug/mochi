...
class NewComponentGenerator
  ...
  def generate(
    ...
    js = String.build do |s|
        ...
        s << "      const target = event.target.closest('[on\\\\:click]');\n"
        s << "      if (target) {\n"
        s << "        const action = target.getAttribute('on:click').replace(/[{}]/g, '');\n"
        s << "        this.rubyComp['$'+action]();\n"
        s << "        this.updateAll();\n"
        s << "      }\n"
        s << "    });\n"
        s << "    this.shadow.addEventListener('change', (event) => {\n"
        s << "      const target = event.target.closest('input[on\\\\:change]');\n"
        s << "      if (target) {\n"
        s << "        const action = target.getAttribute('on:change').replace(/[{}]/g, '');\n"
        s << "        let val = target.value;\n"
        s << "        if (!isNaN(val) && val !== '') val = Number(val);\n"
        s << "        this.rubyComp['$'+action](event, val);\n"
        s << "        this.updateAll();\n"
        s << "      }\n"
        s << "    });\n\n"
